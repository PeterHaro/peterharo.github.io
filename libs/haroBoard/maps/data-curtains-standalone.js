var data_curtain = function (viewer) {
    "use strict";

//__GLOBAL_VARIABLES_
    var dayTracks = true, nightTracks = false, markers = true;
    var trackColor, dayString, monthString, direction, right, up, entityInstance, handler, indices;

    /*  //USE THIS INTIALIZER IF YOU WANT TO ACTUALLY USE THE LIDAR VALUE AS INTENDED WITH THE SATELITE CLOUD DATA AND CURTAINS
    ...
    function visualize(CalipsoData, dateString, time) {
        ...
        for (var m = 0; m < CalipsoData[0].curtains.length; m++) {
            ...
            for (var i = 0; i < CalipsoData[0].curtains[m].sections.length; i++) {
                ...
                if (prevDate != dateString || firstVisualize == 0) {
                    if (CalipsoData[0].curtains[m].orbit == "Daytime")
                        content = "<div id=pb_item_data class=day name=D0C" + m + "S" + i + " onmouseover=data_curtain.hoveredDiv('D0C" + m + "S" + i + "'); onclick=data_curtain.clickedDiv('D0C" + m + "S" + i + "'); onmouseleave=data_curtain.leftDiv('D0C" + m + "S" + i + "');><table><tr><td><img src=" + CalipsoData[0].curtains[m].sections[i].img + " height=35 width=80/></td><td>&nbsp;Start Time: " + CalipsoData[0].curtains[m].sections[i].start_time + "<br>&nbsp;End Time: &nbsp;" + CalipsoData[0].curtains[m].sections[i].end_time + "</td></tr></table></div>";
                    if (CalipsoData[0].curtains[m].orbit == "Nighttime")
                        content = "<div id=pb_item_data class=night name=D0C" + m + "S" + i + " onmouseover=data_curtain.hoveredDiv('D0C" + m + "S" + i + "'); onclick=data_curtain.clickedDiv('D0C" + m + "S" + i + "'); onmouseleave=data_curtain.leftDiv('D0C" + m + "S" + i + "');><table><tr><td><img src=" + CalipsoData[0].curtains[m].sections[i].img + " height=35 width=80/></td><td>&nbsp;Start Time: " + CalipsoData[0].curtains[m].sections[i].start_time + "<br>&nbsp;End Time: &nbsp;" + CalipsoData[0].curtains[m].sections[i].end_time + "</td></tr></table></div>";
    ...
    function isMarkerTime(time, startTime, endTime) {
        if (time != -1) {
            ...
            return !!(selTime.getTime() >= sTime.getTime() && selTime.getTime() <= eTime.getTime());
        } else {
            return false;
        }
    }
